...
export EXECFILE='train_ddpbcp.py'
...
source /users/PCON0023/hamza23/cse5449/miniconda3/bin/activate
...
time srun python3 ${SLURM_SUBMIT_DIR}/${EXECFILE} --epochs=4 --bc=50 --local_world_size=1 >>current.out 2>&1
echo job ended at `date` >>current.out

# I would also change the folder name to match
export SAVEDIR=${SLURM_SUBMIT_DIR}'/tests/bc50_weak_test_4_gpu.'${PBS_JOBID}
mkdir ${SAVEDIR}
