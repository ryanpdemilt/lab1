...
def train(args,local_world_size,rank):
    ...
    if args.no_cuda: # change done here, default for bc_mb is 25
        model = DDP(model, bucket_cap_mb=args.bucket_cap, find_unused_parameters=True)
    else:
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False
        model = DDP(model.cuda(), bucket_cap_mb=args.bucket_cap, find_unused_parameters=True)
